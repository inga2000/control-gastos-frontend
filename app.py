...
fecha = st.date_input("Fecha", value=date.today(),format="DD/MM/YYYY",key="alta_fecha")
monto = st.number_input("Monto", min_value=0.0, step=100.0, key="alta_monto")
tipo = st.selectbox("Tipo", ["Gasto", "Ingreso"], key="alta_tipo")
categoria = st.selectbox("Categoría", ["Fijo", "Variable"], key="alta_categoria")
...
if not datos.empty:
    ...
    # Totales
    total_ingresos = datos_mes.loc[datos_mes["Tipo"] == "Ingreso", "Monto"].sum()
    total_gastos = datos_mes.loc[datos_mes["Tipo"] == "Gasto", "Monto"].sum()
    ...
    if not diario.empty:
        ...
        ef = st.date_input("Fecha", mov["Fecha"].date(),format="DD/MM/YYYY",key=f"edit_fecha_{mov['id']}")
        ...
        ec = st.selectbox("Categoría", ["Fijo", "Variable"], index=0 if mov["Categoría"]=="Fijo" else 1, key=f"edit_categoria_{mov['id']}")
    ...
    gastos_f = datos_mes[(datos_mes["Tipo"]=="Gasto")&(datos_mes["Categoría"]=="Fijo")].groupby(datos_mes["Fecha"].dt.day)["Monto"].sum()
    gastos_v = datos_mes[(datos_mes["Tipo"]=="Gasto")&(datos_mes["Categoría"]=="Variable")].groupby(datos_mes["Fecha"].dt.day)["Monto"].sum()
